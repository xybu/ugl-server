...
class OpauthStrategy{	
	 ...
	 */
	public function __construct($strategy, $env, $f3){
	...
	private function shipToCallback($data, $transport = null){
		...
		switch($transport){
			...
			case 'session':
			default:
				$this->f3->set("SESSION.opauth", $data);
	...
	public static function curl_httpPost($url, $options = null) {
		$ch = curl_init();
		...
		curl_setopt($ch, CURLOPT_USERAGENT,'Ugli-web');
	...
	public static function curl_httpGet($url, $options = null) {
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_AUTOREFERER, true);
		curl_setopt($ch, CURLOPT_VERBOSE, true);
		curl_setopt($ch, CURLOPT_HEADER, true);
	...
	public static function httpRequest($url, $options = null, &$responseHeaders = null){
		...
		if (!empty($options) && is_array($options)){
			...
			//var_dump($options);
			$content = OpauthStrategy::curl_httpPost($url, $options);	
		} else {
			$content = OpauthStrategy::curl_httpGet($url);
